...
def test_strip_trailing_colons():
    # Any accidental trailing ':' tokens should be stripped
    rules = [
        make_rule(
            name="NoColons",
            srcintf="port1",
            dstintf="port2",
            srcaddr="VLAN201: VLAN10:",
            dstaddr="LAN: SRV:",
            service="ALL",
            schedule="always",
            action="accept",
    ...
    assert 'set srcaddr "VLAN201" "VLAN10"' in cli
    assert 'set dstaddr "LAN" "SRV"' in cli
